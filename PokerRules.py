...
def rank_hand(handString):
	...
	if kind4: return CONST_FOUR_OF_A_KIND, kind4, kind1
	
	kind3 = of_a_kind(3)
	kind2 = of_a_kind(2)
		
	if kind3 and kind2: return CONST_FULL_HOUSE, kind3, kind2
	if flush: return CONST_FLUSH, ranks
	if straight: return CONST_STRAIGHT, ranks
	if kind3: return CONST_THREE_OF_A_KIND, kind3, [card for card in ranks if card not in kind3]
	
	secondKind2 = of_a_kind(2, kind2)
	
	if kind2 and secondKind2: return CONST_TWO_PAIR, kind2, secondKind2, kind1
	if kind2: return CONST_PAIR, kind2, [card for card in ranks if card not in kind2]	
